package io.github.cdisvm.runtime.util;
...
public class DefaultFormatSpec {
    ...
    public static DefaultFormatSpec fromSpec(PyStr formatSpec) {
        Matcher matcher = DEFAULT_FORMAT_SPEC_PATTERN.matcher(formatSpec.value());

        if (!matcher.matches()) {
            throw new PyValueError("Invalid format spec: " + formatSpec.value());
        }
    ...
    /**
     * For use by {@link PyStr}, where since Python 3.10, 0 before width do not affect default alignment
     * of strings.
     */
    public static DefaultFormatSpec fromStringSpec(PyStr formatSpec) {
        Matcher matcher = DEFAULT_FORMAT_SPEC_PATTERN.matcher(formatSpec.value());
